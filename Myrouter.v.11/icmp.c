...
void swap_ethernet_addr(struct ether_header *eth  ){
    ...
    for( i =0 ; i< ETHER_ADDR_LEN_MY ; i++){
    	temp_eth[i] = eth->ether_shost[i];
    }

    for( i =0 ; i< ETHER_ADDR_LEN_MY ; i++){
    	eth->ether_shost[i] = eth->ether_dhost[i];
    }
    for( i =0 ; i< ETHER_ADDR_LEN_MY ; i++){
    	eth->ether_dhost[i] = temp_eth[i];
    }
...
void modify_icmp(struct icmphdr *icmp_header ,  int t, int c){
     ...
     icmp_header->checksum = htons(checksum_icmp((unsigned char *)icmp_header, ICMP_HEADER_LEN + ICMP_DATALEN));

}

uint16_t
checksum_icmp (unsigned char *_data, int len)
{
