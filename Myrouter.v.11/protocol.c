...
char *update_packet(char *buffer , uint32_t hdrlen)
{
   ...
   // uint8_t *ether_dest , *ether_source;
    ...
    if(packet_to_me(iph , eth)){
    	...
    	if( iph->ip_p == 1/*ICMP*/ ){
    		myif *myinterface;
    		myinterface = get_interface_by_IP(eth->ether_dhost);
    		if(myinterface == NULL){
    ...
    //    iph->ip_sum = htons(doCheckSum(iph , 20));
        ...
        if((lookup_dest_mac_addr(iph->ip_dst , r_str , dest_eth)) ==0){
        	printf("error while lookup address\n");
...
int send_me(char *infc, const char* buf , uint32_t hdrlen)
{
	//pcap_t *eth_handle;
	myif *out_handle;
